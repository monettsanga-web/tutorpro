...
**Legitimate exception:** `Course` schema is still eligible for review snippets.

✅ **Done —** four `Course` entities (Cambridge/Oxford Primary and Secondary) are now in the
homepage schema, generated by `scripts/prerender-home.mjs`. They carry the Google-required
`name`, `description`, `provider`, and a `hasCourseInstance` with `courseMode: online` and a
paid `Offer`.

### How to add real ratings when they exist

Students rate lessons through `rateCompletedBooking()` in `src/bookings.js`. Once a
programme has genuine ratings, add this to the matching course in the `COURSES` array in
`scripts/prerender-home.mjs`:

```js
aggregateRating: {
  '@type': 'AggregateRating',
  ratingValue: '4.8',   // the real average
  reviewCount: '23',    // the real number of ratings
  bestRating: '5',
  worstRating: '1',
},
```
...
- The numbers must be **genuinely earned** — never estimated or aspirational.
- The rating must also be **visible on the page**, not schema-only. Google requires markup
  to mirror what users actually see.
- Use a dot for decimals (`4.8`, never `4,8`).
- Always declare `bestRating` / `worstRating` so the scale is unambiguous.
