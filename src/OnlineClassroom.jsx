      ...
      <main className="classroom-prejoin">
        ...
        <div className="classroom-prejoin__grid">
          ...
          <section className="prejoin-details">
            ...
            {chinaConnection && <div className="china-classroom-connect-card"><div><Globe size={17} /><strong>China connection mode</strong></div><p>{voovFallbackLink ? 'For China networks, Chrome or Edge plus the VooV backup link gives the most reliable class connection.' : 'Cross-border video from mainland China is restricted, so the peer-to-peer video may not connect. Chat, the lesson board, annotations and shared files all continue to work. Your teacher can add a VooV backup link in their profile for video.'}</p><div><button type="button" onClick={() => setLowBandwidthMode((value) => !value)}>{lowBandwidthMode ? 'Standard video' : 'Low-bandwidth mode'}</button>{voovFallbackLink && <a href={voovFallbackLink} target="_blank" rel="noreferrer">Open VooV backup</a>}</div></div>}
